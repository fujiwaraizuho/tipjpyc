...
export class initialize1644505346587 implements MigrationInterface {
	name = "initialize1644505346587";
	...
	public async up(queryRunner: QueryRunner): Promise<void> {
		...
		await queryRunner.query(
			`CREATE TABLE \`deposit_queue\` (\`id\` bigint NOT NULL AUTO_INCREMENT, \`user_id\` bigint NOT NULL, \`txid\` varchar(255) NOT NULL, \`amount\` int NOT NULL, \`network_type\` enum ('RINKEBY', 'POLYGON', 'MUMBAI') NOT NULL, \`status\` enum ('UNCONFIRM', 'CONFIRMED', 'REJECT') NOT NULL DEFAULT 'UNCONFIRM', \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), UNIQUE INDEX \`IDX_e8305199601ececae2f3b07e4d\` (\`txid\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`
		);
		await queryRunner.query(
			`CREATE TABLE \`deposit_history\` (\`id\` bigint NOT NULL AUTO_INCREMENT, \`deposit_queue_id\` bigint NOT NULL, \`transaction_id\` bigint NOT NULL, \`txid\` varchar(255) NOT NULL, \`amount\` int NOT NULL, \`network_type\` enum ('RINKEBY', 'POLYGON', 'MUMBAI') NOT NULL, \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), UNIQUE INDEX \`IDX_77c9b37da975ba2317c5421405\` (\`txid\`), UNIQUE INDEX \`REL_da7b78bb01478d0893a5efe7bb\` (\`transaction_id\`), UNIQUE INDEX \`REL_e30da3b9f58368a7140c282988\` (\`deposit_queue_id\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`
		);
		await queryRunner.query(
		...
		await queryRunner.query(
			`ALTER TABLE \`deposit_queue\` ADD CONSTRAINT \`FK_3657c28db8136b9371596b633d8\` FOREIGN KEY (\`user_id\`) REFERENCES \`user\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`
		);
		await queryRunner.query(
		...
		await queryRunner.query(
			`ALTER TABLE \`deposit_history\` ADD CONSTRAINT \`FK_e30da3b9f58368a7140c282988a\` FOREIGN KEY (\`deposit_queue_id\`) REFERENCES \`deposit_queue\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`
		);
	}

	public async down(queryRunner: QueryRunner): Promise<void> {
		await queryRunner.query(
			`ALTER TABLE \`deposit_history\` DROP FOREIGN KEY \`FK_e30da3b9f58368a7140c282988a\``
		);
		await queryRunner.query(
			`ALTER TABLE \`deposit_history\` DROP FOREIGN KEY \`FK_da7b78bb01478d0893a5efe7bbb\``
		);
		await queryRunner.query(
			`ALTER TABLE \`deposit_queue\` DROP FOREIGN KEY \`FK_3657c28db8136b9371596b633d8\``
		);
		await queryRunner.query(
		...
		await queryRunner.query(
			`DROP INDEX \`REL_e30da3b9f58368a7140c282988\` ON \`deposit_history\``
		);
		await queryRunner.query(
		...
		await queryRunner.query(
			`DROP INDEX \`IDX_e8305199601ececae2f3b07e4d\` ON \`deposit_queue\``
		);
		await queryRunner.query(`DROP TABLE \`deposit_queue\``);
		await queryRunner.query(
